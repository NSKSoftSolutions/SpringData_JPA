...
public interface EmployeeRepositorie extends JpaRepository<Employee, Integer> {
	...
	public Employee findByEid(int empId);

	@Transactional
	@Modifying
	@Query("update Employee e set e.name=?1 where e.eid=?2")
	public int updateEmployeeName(String empName, int eId);

	
	@Transactional
	@Modifying
	@Query("Delete from Employee e where e.eid=?1")
	public int deleteEmployeeDetails(int eid);
